...
sim_RN_Gaus = function(J, rep_z, rep_W, l_es, u_es,
  ...
  min = l_es 
  max = u_es
  b = runif(2,min=min,max=max) * sample(c(-1,1), 2, prob = c(0.5,0.5), replace = T)
  q = runif(2,min=min,max=max) * sample(c(-1,1), 2, prob = c(0.5,0.5), replace = T)
  
  #random effects (z)
  sd_P_z = rep(sqrt(V_P), 2)
  LP = t(chol(rethinking::rlkjcorr(1,2,lkj_cor))) #RN parameter correlations
  std_dev_z = cbind(rnorm(J,0,1),rnorm(J,0,1)) #individual-level RN deviations
  std_dev_z = matrix(scale(std_dev_z), ncol = 2)
  zp = std_dev_z %*% t( diag(sd_P_z) %*% LP )
  
  #RN parameters
  zp_mu = zp[,1] #intercepts
  zp_sigma = zp[,2] #residuals
  
  #sample environmental states
  x = rnorm(N_z, 0, 1)
  
  #phenotype expectations and dispersions
  z_eta = mu_0z + zp_mu[ind_z] + beta_z*x
  ...
  if(rep_W > 1){
    ...
    W_eta = theta_0w + W_0[ind_W]+
      ...
      q[2]*(zp_sigma[ind_W] * zp_sigma[ind_W])   
